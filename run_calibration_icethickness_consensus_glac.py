...
#%% ----- MANUAL INPUT DATA -----
rgi_glac_number = '01.15645' # '01.15645' for Kennicott Glacier, '01.26722' for Root Glacier
vol_offset = 0 # difference between consensus volume (~2000) and historical volume, in km3
    # vol_offset is 4.228983 km3 for Kennicott, 1.155888 km3 for Root
...
def reg_vol_comparison(gdirs, mbmods, a_multiplier=1, fs=0, v_offset=0, debug=False):
    ...
    for nglac, gdir in enumerate(gdirs):
        ...
        if os.path.exists(gdir.get_filepath('consensus_mass')):
            ...
            reg_vol_km3_consensus += (consensus_km3 + v_offset)
...
reg_vol_km3_mod, reg_vol_km3_con = reg_vol_comparison(gdirs, mbmods, a_multiplier=a_multiplier, fs=fs, 
                                                        v_offset=vol_offset, debug=False)
# Lower bound
...
# ===== EXPORT RESULTS =====
glena_cns = ['O1Region', 'glac', 'count', 'glens_a_multiplier', 'fs', 'reg_vol_km3_consensus', 'reg_vol_km3_modeled']
glena_df_single = pd.DataFrame(np.zeros((1,len(glena_cns))), columns=glena_cns)
glena_df_single.loc[0,:] = [float(rgi_glac_number[:2]), float(rgi_glac_number[3:]), main_glac_rgi_subset.shape[0], 
                            a_multiplier_opt, fs, reg_vol_km3_con, reg_vol_km3_mod]
...
if os.path.exists(pygem_prms.glena_reg_fullfn):
    ...
    conds = (glena_df.O1Region == float(rgi_glac_number[:2])) & (glena_df.glac == float(rgi_glac_number[3:]))
    glena_idx = np.where(conds)[0]
